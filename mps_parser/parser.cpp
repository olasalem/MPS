...
QVector<inst> parser::parseFile(QFile & instFile) {
    QVector <inst> instMem; //to be returned
    inst temp; //current instruction that is being formed
    ...
    if(instFile.open(QFile::ReadOnly)) { //todo : open file in read only format
        ...
        while(!fileStream.atEnd()) {
            ...
            if(inst_3Op.indexIn(line) != -1) { //match 3operands instruction
                temp.instruction = inst_3Op.cap(0);
                ...
                temp.instruction = inst_2Op.cap(0);
                ...
                temp.instruction = inst_1Op.cap(0);
                ...
                if(temp.instType == "JR") { //JR
                    ...
                    temp.instruction = inst_0Op.cap(0);
                    temp.instType = inst_1Op.cap(1);
